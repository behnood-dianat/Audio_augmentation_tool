...
@tf.function
def sparse_warp(mel_spectrogram, time_warping_para=100):
    ...
    src_ctr_pt_freq = tf.range(v)  # control points on freq-axis
    ...
    warped_image, _ = sparse_image_warp(mel_spectrogram,
                                        ...
                                        dest_control_point_locations,num_boundary_points=1)
    return warped_image

@tf.function
def frequency_masking(mel_spectrogram, v, frequency_masking_para=90, frequency_mask_num=2):
    """Spec augmentation Calculation Function.
...
@tf.function
def time_masking(mel_spectrogram, tau, time_masking_para=25, time_mask_num=2):
    """Spec augmentation Calculation Function.
...
def visualization_tensor_spectrogram(mel_spectrogram, title):
    ...
    plt.ion()
    plt.figure(figsize=(10, 4))
    librosa.display.specshow(librosa.power_to_db(mel_spectrogram[0, :, :, 0], ref=np.max), y_axis='mel', fmax=1800, x_axis='time')
