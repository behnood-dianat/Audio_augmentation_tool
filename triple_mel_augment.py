	...
	def augment_loop(sounds1,sounds2,sounds3,sounds4,output_path,names,batch_num,minimum):
		...
		for j in range(len(sounds1)):
			...
			if (len(sounds1[j])/sampling_rate) >= minimum:
				...
				mfccs4 = librosa.feature.mfcc(y=sounds4[j],sr=sampling_rate,hop_length=32,n_mels=32,n_mfcc=32,fmin=400,fmax=fmax)
				...
				concatenated = np.concatenate((S_dB_2,S_dB_3,S_dB_4))
